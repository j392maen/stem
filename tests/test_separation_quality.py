...
from stemapp.models import SeparationJob, SeparationPreset, Stem
...
def _plan(route: str | None, karaoke_input: str = "vocals", karaokes: int = 1) -> PresetPlan:
    kara = [KARAOKE_BS_FRAZER, KARAOKE_BS_ANVUEW][:karaokes]
    steps = [
        StepSpec(1, SW, "SW", "mixture", "multistem"),
        StepSpec(2, KIM_VOCALS, "Kim", "mixture", "vocals"),
    ] + [StepSpec(3 + i, m, m, karaoke_input, "karaoke") for i, m in enumerate(kara)]
...
# standard 相当では vocals の平均 0.35 → 上位 stem の合計 1.02 → 残差 −0.02。
@pytest.mark.parametrize(
    ("route", "vocals", "other"),
    [
        (None, 0.35, 0.10),  # 既定（other）: 今までどおり
        ("other", 0.35, 0.10),
        ("vocals", 0.33, 0.12),  # 残差を vocals に
        # split: ボーカルの平均による差（0.30 − 0.35 = −0.05）を vocals、残り（+0.03）を other
        ("split", 0.30, 0.15),
    ],
)
def test_residual_routes_keep_sums(
    tmp_path: Path, mix: np.ndarray, route: str | None, vocals: float, other: float
) -> None:
    ...
    assert out.residual_rms_db == pytest.approx(
        20 * np.log10(np.sqrt(np.mean((mix.astype(np.float64) * 0.02) ** 2))), abs=0.01
    )
...
def test_karaoke_on_mixture(tmp_path: Path, mix: np.ndarray, route: str) -> None:
    ...
    assert [c.role for c in sep.calls] == ["multistem", "vocals", "karaoke", "karaoke"]
...
def test_experimental_preset_contents(seeded: Session) -> None:
    ...
    base = [(SW, "mixture", "multistem"), (KIM_VOCALS, "mixture", "vocals")]
    assert desc("exp_resid_vocals") == ("vocals", [*base, (KARAOKE_BS_FRAZER, "vocals", "karaoke")])
